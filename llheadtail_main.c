...
int processFastaHeadTail(char *filename, double *timeTaken)
{
    FILE *fp;
    FASTArecord fRecord;
...
int processFastaRepeatedlyHeadTail(char *filename,long repeatsRequested)
{
    ...
    for (i = 0; i < repeatsRequested; i++) {
        status = processFastaHeadTail(filename, &timeThisIterationInSeconds);
...
int main(int argc, char **argv)
{
    ...
    recordsProcessed = processFastaRepeatedlyHeadTail(argv[i],
    repeatsRequested);
