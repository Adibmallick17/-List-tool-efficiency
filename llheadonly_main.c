...
int
processFastaWithHead(char *filename, double *timeTaken)
{
    FILE *fp;
    FASTArecord fRecord;
    int lineNumber = 0, recordNumber = 0, status, eofSeen = 0;
    ...
    do {
        ...
        * we know something is happening */
        ...
        eofSeen = 1;
            ...
            lineNumber = lineNumber + status;
            recordNumber++;
            head = llPrepend(head, llNewNode(fRecord.description, &fRecord));
...
int
processFastaRepeatedlyWithHead(char *filename,long repeatsRequested)
{
    ...
    status = processFastaWithHead(filename, &timeThisIterationInSeconds);
...
main(int argc, char **argv)
{
    ...
    recordsProcessed = processFastaRepeatedlyWithHead(argv[i],
    repeatsRequested);
