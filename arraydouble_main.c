...
int processFastaWithArray(char *filename, double *timeTaken) {
    FILE *fp;
    FASTArecord fRecord;
    int lineNumber = 0, recordNumber = 0, status, eofSeen = 0,size = 1;
    clock_t startTime, endTime;
    FASTArecord *record = (FASTArecord *)malloc(size * sizeof(FASTArecord));
        ...
        * we know something is happening */
        ...
        if (status == 0) {
            ...
            if (recordNumber >= size) {
                size = size * 2;
                record = (FASTArecord *)realloc(record, size * sizeof(FASTArecord));
            }
            //fastaPrintRecord(stdout, &fRecord);
            record[recordNumber - 1] = fRecord;
    ...
    int wastedSpace = ((size - recordNumber) * sizeof(FASTArecord));
    printf(" %d FASTA records -- %d allocated (%.3f%% waste)\n", recordNumber, size, (wastedSpace / (double)(size * sizeof(FASTArecord)) * 100.0));
...
int
processFastaRepeatedlyWithArray(
        char *filename,
        long repeatsRequested
    )
{
    double timeThisIterationInSeconds, totalTimeInSeconds = 0;
    int minutesPortion, status;
    long i;

    for (i = 0; i < repeatsRequested; i++) {
        status = processFastaWithArray(filename, &timeThisIterationInSeconds);
...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        if (argv[i][0] == '-') {
            ...
            recordsProcessed = processFastaRepeatedlyWithArray(argv[i],repeatsRequested);
